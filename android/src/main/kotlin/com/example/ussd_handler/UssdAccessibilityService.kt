...
class UssdAccessibilityService : AccessibilityService() {
    ...
    companion object {
        ...
        @Volatile
        private var currentMultiSessionEvent: android.view.accessibility.AccessibilityEvent? = null
    ...
    override fun onServiceConnected() {
        ...
        val info =
                AccessibilityServiceInfo().apply {
                    ...
                    packageNames = arrayOf("com.android.phone", "com.android.server.telecom")
                }
    ...
    private fun isUssdSystemDialog(className: String?, packageName: String?): Boolean {
        ...
        val ussdPackages =
                listOf("com.android.phone", "com.android.dialer", "telephony", "ussd", "mmi")
                ...
                synchronized(this) {
                    ...
                    if (ussdResponseCallback != null && !responseAlreadySent) {
                        ...
                        try {
                            ...
                            // Do not reset responseAlreadySent in case of error to avoid multiple
                            // attempts
                        }
    ...
    fun clickUssdButtonImproved(buttonText: String): Boolean {
        ...
        return try {
            ...
            // Strategy 4: Search for any button that may be a confirmation button based on position
            // or class
    ...
    private fun logAllClickableNodes(node: AccessibilityNodeInfo, depth: Int = 0) {
        try {
            ...
            if (node.isClickable) {
                ...
                Log.d(
                        TAG,
                        "${indent}CLICKABLE: text='$text', desc='$contentDesc', class='$className'"
                )
            }
    ...
    private fun findButtonByPartialText(
    ...
    private fun findButtonByContentDescription(
            node: AccessibilityNodeInfo,
            targetText: String
    ): AccessibilityNodeInfo? {
    ...
    fun writeTextToUssdDialog(text: String): Boolean {
        ...
        return try {
            ...
            if (editTextNode != null) {
                ...
                Log.d(
                        TAG,
                        "Field properties: editable=${editTextNode.isEditable}, focusable=${editTextNode.isFocusable}, enabled=${editTextNode.isEnabled}"
                )
    ...
    private fun findEditTextNode(node: AccessibilityNodeInfo): AccessibilityNodeInfo? {
        Log.d(
                TAG,
                "Searching for input field in node: ${node.className}, editable=${node.isEditable}, focusable=${node.isFocusable}, enabled=${node.isEnabled}"
        )
        ...
        if (hintText != null &&
                        (hintText.contains("input") ||
                                hintText.contains("enter") ||
                                hintText.contains("write") ||
                                hintText.contains("type") ||
                                hintText.contains("type"))
        ) {
    ...
    /**
     * Automatically closes the USSD dialog after capturing its content to create a completely
     * headless experience
     */
    ...
    private fun findAndClickDismissibleButton(rootNode: AccessibilityNodeInfo): Boolean {
        return try {
            ...
            val dismissibleTexts =
                    listOf("OK", "Accept", "Aceptar", "Close", "Done", "Finish", "Exit")
    ...
    private fun findAndClickAnyDismissButton(rootNode: AccessibilityNodeInfo): Boolean {
        try {
            ...
            if (rootNode.isClickable &&
                            rootNode.className?.toString()?.contains("Button", ignoreCase = true) ==
                                    true
            ) {
                val buttonText = rootNode.text?.toString()?.trim()
                if (buttonText != null && buttonText.length <= 15
                ) { // Avoid very long texts that are not buttons
    ...
    private fun isRealUssdDialog(rootNode: AccessibilityNodeInfo): Boolean {
        try {
            ...
            if (packageName != null &&
                            (packageName.contains("flutter") || packageName.contains("example"))
            ) {
                Log.d(
                        TAG,
                        "DETECTED: Flutter UI detected (package: $packageName), NOT a system USSD dialog"
                )
                return false
            }

            // Search for system USSD dialog indicators
            val ussdIndicators = listOf("com.android.phone", "dialer", "telephony", "ussd", "mmi")
    ...
    private fun hasSystemDialogCharacteristics(node: AccessibilityNodeInfo): Boolean {
        try {
            ...
            Log.d(
                    TAG,
                    "Dialog characteristics - EditText: $hasEditText, System buttons: $hasSystemButtons, Is system dialog: $isSystemDialog"
            )
    ...
    private fun checkDialogCharacteristics(
            node: AccessibilityNodeInfo,
        ...
        if (node.isClickable && node.className?.toString()?.contains("Button") == true) {
            ...
            val systemButtonTexts =
                    listOf("OK", "Cancel", "Send", "Accept", "Aceptar", "Cancelar", "Enviar")
            val isSystemButton =
                    systemButtonTexts.any { systemText ->
                        text.equals(systemText, ignoreCase = true) ||
                                desc.equals(systemText, ignoreCase = true)
                    }
                ...
                .postDelayed(
                        {
                            if (waitingForUssdDialog) {
                                Log.w(
                                        TAG,
                                        "TIMEOUT: No system USSD dialog detected in ${ussdDialogWaitTimeout}ms"
                                )
                                waitingForUssdDialog = false
     ...
     *
     * This method configures the service to keep a USSD dialog open and allow multiple interactions
     * with it.
     *
                    ...
                    .postDelayed(
                            {
                                synchronized(this) {
                                    if (isMultiSessionActive &&
                                        ...
                                        Log.w(
                                                TAG,
                                                "TIMEOUT: No real USSD response received in 15 seconds"
                                        )
                                        responseAlreadySent = true
                                        try {
                                            multiSessionResponseCallback?.invoke(
                                                    "TIMEOUT: No response received from operator"
                                            )
                                        } catch (e: Exception) {
    ...
    fun sendMessageInSession(message: String): Boolean {
        return try {
            ...
            if (isProgressDialog(rootNode)) {
                Log.w(
                        TAG,
                        "Progress dialog detected, cannot send message yet. Waiting for real menu..."
                )
                return false
            }
            ...
            synchronized(this) { responseAlreadySent = false }
                                        ...
                                        .postDelayed(
                                                {
                                                    synchronized(this) {
                                                        if (isMultiSessionActive &&
                                                                        !responseAlreadySent &&
                                                                        multiSessionResponseCallback !=
                                                                                null
                                                        ) {
                                                            Log.w(
                                                                    TAG,
                                                                    "TIMEOUT: No response received after sending message '$message'"
                                                            )
                                                            responseAlreadySent = true
                                                            try {
                                                                multiSessionResponseCallback
                                                                        ?.invoke(
                                                                                "TIMEOUT: No response after sending '$message'"
                                                                        )
                                                            } catch (e: Exception) {
                                                                Log.e(
                                                                        TAG,
                                                                        "Error sending message timeout: ${e.message}"
                                                                )
                                                            }
                                                        }
                                                    }
                                                ...
                                                10000
                                        ) // 10 second timeout for messages
                            },
                            500
                    ) // 500ms delay
    ...
    private fun handleMultiSessionUssdContent(rootNode: AccessibilityNodeInfo?) {
        ...
        try {
            ...
            if (ussdText.isNotEmpty()) {
                ...
                val isProgressMessage = progressMessages.any { ussdText.lowercase().contains(it) }

                if (isProgressMessage) {
                    Log.d(
                            TAG,
                            "MULTI-SESSION: Progress message detected, waiting for real response..."
                    )
                    return
                }
    ...
    /**
     * Debug function to list all nodes of a dialog Useful for diagnosing why the input field is not
     * found
     */
    private fun debugLogAllNodes(node: AccessibilityNodeInfo, depth: Int) {
        try {
            ...
            Log.d(
                    TAG,
                    "${indent}[D$depth] $className | text='$text' | desc='$contentDesc' | hint='$hintText' | " +
    ...
    /**
     * Checks if the current dialog is a progress dialog (which only shows "Executing USSD code..."
     * and has no input field)
     */
    private fun isProgressDialog(rootNode: AccessibilityNodeInfo): Boolean {
        try {
            ...
            if (isProgress) {
                Log.d(
                        TAG,
                        "✋ DETECTED: Progress dialog - text='$dialogText', hasProgressBar=$hasProgressBar, hasInputField=$hasInputField"
                )
            }
